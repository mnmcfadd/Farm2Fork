    ...
    for person in user_roles_list:
        ...
        if person[1] == "Distributor":
            for i in range(round(random.random()*15)):
                ...
                if buy_product == 1:
                    product = farmer_inventory.keys()[randint(0, len(farmer_inventory.keys())-1)]
                    product = products[math.floor(random.random()*len(products))]
                    if product in farmer_inventory:
                        options = farmer_inventory[product]
                        if type(options) == list:
                            farmer = options[math.floor(random.random()*len(options))]
                            quantity = math.floor(random.random()*farmer[1])
                            ...
                            farmer_inventory[product]= farmer_inventory[product].remove(farmer)
                            farmer_inventory[product]= farmer_inventory[product].append((farmer[0], farmer[1]-quantity))
                
with open ("F2F_Data.SQL", "a") as f:
    for person in user_roles_list:
        #customer purchases from distributor 
        if person[1] == "Customer":
            for i in range(round(random.random())):
                product = products[math.floor(random.random()*len(products))]
                if product in distributor_inventory:
                    options = distributor_inventory[product]
                    if type(options) == list:
                        distributor = options[math.floor(random.random()*len(options))]
                        ...
                        distributor_inventory[product]= distributor_inventory[product].remove(distributor)
